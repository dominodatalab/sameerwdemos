...
class MNISTDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.df_train = datasets.MNIST(
            "/tmp/minst/dataset", download=True, train=True, transform=self.transform
        )
        self.df_train, self.df_val = random_split(self.df_train, [55000, 5000])
        self.df_test = datasets.MNIST(
            "/tmp/minst/dataset", download=True, train=False, transform=self.transform
        )
...
if __name__ == "__main__":
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath='/tmp/pytorch/', save_top_k=1, verbose=True, monitor="val_loss", mode="min"
    )
    ...
    mlflow.set_experiment("DEMO-MINST-AUTOLOGGING")
    mlflow.start_run()
